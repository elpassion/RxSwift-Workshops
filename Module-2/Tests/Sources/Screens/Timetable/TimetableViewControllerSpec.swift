    ...
    override func spec() {
        describe("TimetableViewController") {
            ...
            context("when service throws an error") {
                beforeEach {
                    serviceStub.stubbedError = TimetableServiceError.unknown
                }

                it("should NOT throw an error when data is binded to tableView") {
                    expect { _ = sut.view }.notTo(throwAssertion())
                }

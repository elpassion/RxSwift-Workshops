...
class TimetableViewController: UIViewController {
    ...
    private func setUpTableViewDataSource() {
        let entries: Observable<[TimetableEntry]> = Observable.merge(
            Observable.just(()),
            refreshControl.rx.controlEvent(.valueChanged).asObservable()
        ).flatMap { [unowned self] in self.timetableService.timetableEntries.trackActivity(self.activityIndicator) }

        Observable
            .combineLatest(entries, selectedFilter)
